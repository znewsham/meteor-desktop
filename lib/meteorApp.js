...
export default class MeteorApp {
    ...
    async acquireIndex() {
        ...
        const ddpUrl = this.$.env.options.ddpUrl.replace(/\/$/, '') || `http://127.0.0.1:${port}`;
        const res = await fetch(`${ddpUrl}/__cordova/index.html`);
    ...
    async acquireManifest() {
        ...
        const res = await fetch(
            `${ddpUrl}/__cordova/manifest.json?meteor_dont_serve_index=true`
        );
        const text = await res.text();
